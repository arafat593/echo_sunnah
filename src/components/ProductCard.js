...
export default function ProductCard({ product, onCardClick, onAddToCart, onBuyNow, className = "", ...props }) {
  return (
    ...
    >
      ...
      <div className="mt-3 sm:mt-4 pt-2.5 sm:pt-3.5 border-t border-slate-100/80 flex flex-col gap-3">
        <div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 w-full">
          <button
            ...
            className="flex-1 border border-emerald-600 text-emerald-800 hover:bg-emerald-50 py-2 sm:py-2.5 px-2.5 rounded-xl text-xs sm:text-sm font-black transition-all flex items-center justify-center gap-1 active:scale-[0.97]"
            title="Add to Cart"
          >
          ...
          {onBuyNow && (
            <button
              ...
              className="flex-1 bg-gradient-to-r from-emerald-700 to-teal-800 hover:from-emerald-650 hover:to-teal-700 text-white py-2 sm:py-2.5 px-3 rounded-xl text-xs sm:text-sm font-black transition-all shadow-[0_4px_12px_rgba(4,120,87,0.14)] hover:shadow-[0_6px_16px_rgba(4,120,87,0.22)] active:scale-[0.97] text-center whitespace-nowrap"
            >
              Buy Now
            </button>
          )}
