...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-gradient-to-b from-emerald-950 via-emerald-900/90 to-slate-950 text-white pt-6 md:pt-24 pb-4 md:pb-8 mt-6 md:mt-16 overflow-visible">
      ...
      <div className="absolute top-0 left-0 right-0 w-full overflow-hidden leading-none transform -translate-y-[99%] z-20 pointer-events-none">
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-[20px] md:h-[45px] text-emerald-950 fill-current">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Main Columns Grid */}
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-6 md:gap-8 pb-4 md:pb-16">
          ...
          <div className="col-span-2 md:col-span-1 bg-white/5 border border-white/10 rounded-3xl p-4 md:p-6 backdrop-blur-md shadow-2xl space-y-3 md:space-y-6 hover:border-emerald-500/30 transition-colors duration-300">
            <Link href="/" className="flex items-center gap-3 group">
              <div className="bg-emerald-800 p-1.5 rounded-2xl shadow-lg group-hover:scale-105 group-hover:rotate-3 transition-all duration-300 relative w-12 h-12 flex items-center justify-center shrink-0">
                <Image src="/echo_sunnah_logo.png" alt="Echo Sunnah" width={38} height={38} className="object-contain" style={{ width: "auto", height: "auto" }} />
              </div>
            ...
            <p className="text-[11px] sm:text-sm text-emerald-100/70 leading-relaxed font-medium">
          ...
          <div className="col-span-1 pl-0 lg:pl-8">
            <h4 className="text-xs sm:text-sm font-black uppercase tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-amber-200 to-amber-400 mb-3 md:mb-6 pb-2 border-b border-white/10 inline-block">
              Quick Links
            </h4>
            <ul className="space-y-1.5 md:space-y-3.5 text-[11px] sm:text-sm font-semibold">
              <li>
                <Link href="/ruqyah" className="group flex items-center gap-2 text-emerald-100/70 hover:text-amber-300 transition-all duration-300">
                  <span className="w-1 h-1 rounded-full bg-emerald-500 group-hover:bg-amber-400 group-hover:scale-125 transition-all"></span>
                  <span>Ruqyah Therapy (রুকইয়াহ)</span>
                </Link>
              </li>
              <li>
                <Link href="/hijama" className="group flex items-center gap-2 text-emerald-100/70 hover:text-amber-300 transition-all duration-300">
                  <span className="w-1 h-1 rounded-full bg-emerald-500 group-hover:bg-amber-400 group-hover:scale-125 transition-all"></span>
                  <span>Hijama Cupping (হিজামা)</span>
                </Link>
              </li>
              <li>
                <Link href="/shop" className="group flex items-center gap-2 text-emerald-100/70 hover:text-amber-300 transition-all duration-300">
                  <span className="w-1 h-1 rounded-full bg-emerald-500 group-hover:bg-amber-400 group-hover:scale-125 transition-all"></span>
                  <span>Wellness Shop (শপ)</span>
                </Link>
              </li>
              <li>
                <Link href="/academy" className="group flex items-center gap-2 text-emerald-100/70 hover:text-amber-300 transition-all duration-300">
                  <span className="w-1 h-1 rounded-full bg-emerald-500 group-hover:bg-amber-400 group-hover:scale-125 transition-all"></span>
                  <span>Training Academy (একাডেমি)</span>
                </Link>
              </li>
              <li>
                <Link href="/about" className="group flex items-center gap-2 text-emerald-100/70 hover:text-amber-300 transition-all duration-300">
                  <span className="w-1 h-1 rounded-full bg-emerald-500 group-hover:bg-amber-400 group-hover:scale-125 transition-all"></span>
          ...
          <div className="col-span-1">
            ...
            <ul className="space-y-2 md:space-y-4.5 text-[11px] sm:text-sm text-emerald-100/80 font-medium">
              <li className="flex items-start gap-2 group">
                <div className="bg-emerald-900/60 p-1.5 rounded-xl text-amber-400 border border-emerald-800/40 group-hover:bg-amber-400 group-hover:text-emerald-950 transition-colors duration-300 shrink-0">
              ...
              <li className="flex items-center gap-2 group">
                ...
                <a href="mailto:info@echosunnah.com" className="hover:text-amber-305 transition-colors pt-0.5 font-semibold">
                  info@echosunnah.com
                </a>
              </li>
              <li className="flex items-center gap-2 group">
        ...
        {/* Bottom Area */}
        <div className="pt-4 md:pt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-3 md:gap-4 text-[11px] sm:text-xs font-semibold text-emerald-200/50">
          <span className="text-center sm:text-left">© {new Date().getFullYear()} Echo Sunnah. All rights reserved. Reviving Prophetic Wellness.</span>

          <div className="flex items-center gap-4 md:gap-6">
            <div className="flex gap-4">
              <Link href="/terms" className="hover:text-amber-305 hover:underline">Terms</Link>
              <Link href="/privacy" className="hover:text-amber-305 hover:underline">Privacy</Link>
            </div>

            <button
              onClick={scrollToTop}
              className="group flex items-center gap-1.5 bg-emerald-900/40 hover:bg-amber-400 text-emerald-100 hover:text-emerald-950 px-3 py-1.5 rounded-xl border border-emerald-800/60 hover:border-amber-400 text-xs font-bold transition-all duration-300 shadow-lg active:scale-95"
            >
